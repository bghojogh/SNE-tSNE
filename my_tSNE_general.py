...
class My_tSNE_general:

    def __init__(self, X, y=None, n_components=2, learning_rate=0.1, learning_rate_forDegree=0.1, max_iterations=1000, 
        ...
        self.learning_rate_forDegree = learning_rate_forDegree
        self.early_exaggeration = early_exaggeration

    def fit_transform(self, continue_from_which_iteration=None):
        path_to_save = './saved_files/tSNE_general_degrees/'
        if continue_from_which_iteration is not None:
            ...
            paths_ = glob.glob(path_to_save+'degree/*')
            paths_ = [path_.split("\\")[-1] for path_ in paths_]
            paths_ = [path_.split(".")[0] for path_ in paths_]
            name_of_variable = [path_ for path_ in paths_ if "itr"+str(continue_from_which_iteration) in path_][0]
            degree_ = self.load_variable(name_of_variable=name_of_variable, path=path_to_save+'degree/')
        else:
        ...
        degree_iters = np.zeros((self.step_checkpoint, 1))
        update = 0
        if continue_from_which_iteration is None:
            degree_ = 1
        while True:
            ...
            ######## optimization for embedded points:
            ...
            d_squared_of_all_devidedByDegree = d_squared_of_all * (1 / degree_)
            d_squared_of_all_plus1 = d_squared_of_all_devidedByDegree + 1
            d_squared_of_all_plus1_inverse = np.reciprocal(d_squared_of_all_plus1) #--> element-wise inverse
            d_squared_of_all_plus1_inverse_poweredToDegree = np.power(d_squared_of_all_plus1_inverse, 0.5*(degree_+1)) #--> element-wise power
            denominator = np.sum(d_squared_of_all_plus1_inverse_poweredToDegree)
            denominator = denominator - np.sum(np.diag(d_squared_of_all_plus1_inverse_poweredToDegree)) #--> remove the diagonal elements
            q_matrix = np.zeros((self.n_training_images, self.n_training_images))
            for sample_index1 in range(self.n_training_images):
                for sample_index2 in range(self.n_training_images):
                    if sample_index1 != sample_index2:
                        numerator = d_squared_of_all_plus1_inverse_poweredToDegree[sample_index1, sample_index2]
            ...
            ######## optimization for degree_:
            numerator1 = -(1 + degree_) * d_squared_of_all
            denominator1 = 2 * (degree_**2) * (1 + ((1/degree_)*d_squared_of_all))
            term1 = numerator1 / denominator1
            term2 = 0.5 * np.log(1 + ((1/degree_)*d_squared_of_all))
            term1_large = term1 + term2
            gradient_for_degree = 0
            for sample_index1 in range(self.n_training_images):
                for sample_index2 in range(self.n_training_images):
                    if sample_index1 != sample_index2:
                        p_ij = p_matrix_symmetric_used[sample_index1, sample_index2]
                        q_ij = q_matrix[sample_index1, sample_index2]
                        gradient_for_degree += term1_large[sample_index1, sample_index2] * (p_ij - q_ij)
            update_for_degree = - (self.learning_rate_forDegree * gradient_for_degree)
            # degree_ = degree_ + update_for_degree
            # print("the updated degree (float): " + str(degree_))
            # degree_ = max(int(degree_), 1)
            # print("the updated degree: " + str(degree_))
            if update_for_degree > 0:
                degree_ = degree_ + 1
            else:
                degree_ = degree_ - 1
            print("the updated degree: " + str(degree_))
            ...
            degree_iters[index_to_save] = degree_
            # save the information at checkpoints:
            if (iteration_index+1) % self.step_checkpoint == 0:
                ...
                self.save_variable(variable=degree_, name_of_variable="degree_itr"+str(iteration_index)+"_cp"+str(checkpoint_index), path_to_save=path_to_save+'degree/')
                self.save_np_array_to_txt(variable=degree_iters, name_of_variable="degree_iters_itr"+str(iteration_index)+"_cp"+str(checkpoint_index), path_to_save=path_to_save+'degree_txt/')
                if self.y is not None:
